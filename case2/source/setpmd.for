...
* Modified:Liesje Mommer                                               *
* Date   : 7-March-1997, 20-1-1999                                     *
* Version: 1.1                                                         *
...
*         CALL ERROR ('SETPMD','unimplemented surface value')
*      END IF
...
*     and estimated temperature difference
      ...
      ETRD = (VPSL*RDN)/(LHVAP*(VPSL + PSCH*(1+RCAN/RAE)))
      ETAE = ((RHOCP*VPD)/RAE)/(LHVAP*(VPSL + PSCH*(1+RCAN/RAE)))
      ETD  = ETRD+ETAE
...
*         INLOOP = 0
*         EQUIL  = .FALSE.
*10       IF (INLOOP.EQ.0 .OR. .NOT.EQUIL) THEN
*            DT = (DT+DTN)/2.
